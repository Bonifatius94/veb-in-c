...
7. **Consistency check.** Before showing the plan to the user, re-read it and verify every concrete claim against the code: trace each cited `file:line` reference, sanity-check memory/size budgets with the actual `sizeof`/universe math, trace any asserted behavioral traces (e.g. "successor(3) on `{5,200}` hits line 452") through the relevant function by hand, and confirm counts/names are internally consistent (test count math, naming parity with existing tests, registration in `main`). Fix what you find in `./plans/<N>.md`, then report the verifications performed and any corrections made. This step is mandatory — a plan that looks right but has an off-by-one line reference or wrong memory estimate wastes the `/implement` session.
8. Show the plan to the user and point them at `./plans/<N>.md`. Iterate on that file until they approve. Then **replace the issue body** with it: `gh issue edit <N> --body-file ./plans/<N>.md`. The refined plan *is* the new issue body — not a comment on top of the draft. Leave `./plans/<N>.md` in place after posting; the user may want to reference it during `/implement`. Do not save plans anywhere else (no hidden memory, no scratch files outside `plans/`).
