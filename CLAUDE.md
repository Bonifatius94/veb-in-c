...
test/unit_tests.c           property tests (u = 4096 only today)
test/sorting_benchmark.c    vEB-sort vs qsort on 500k dense keys
test/CMakeLists.txt         targets: UnitTests, SortingBenchmark
CMakeLists.txt              registers both with CTest
...
- **Memory-efficient root** (`is_memeff_root` in `_vebtree_init`): at the root, `lower_bits` is pinned to `VEBTREE_LEAF_BITS = 6`, so the root's locals are always leaves. This avoids the naive √u split blowing up memory at the top. Non-root nodes use `vebtree_lower_bits(u) = u / 2`.
...
- `VEBTREE_FLAG_LAZY` is defined but unimplemented. `_vebtree_init` skips `_init_subtrees` when lazy, leaving `locals`/`global` as NULL — any insert/delete on a lazy tree segfaults today. Finishing this is the headline roadmap item (#3).
- `vebtree_predecessor` is stubbed with `assert(false)` (#15).
...
- Inserting a duplicate key or deleting an absent key corrupts the structure — no guard today (#18).
- `malloc` caps at ~15 MB in one chunk on some platforms, which limits universes to ≈ 2^19 keys (#20). Partly obviated once lazy allocation lands.
